	...
	if (!update_tx) {
		if (ptx->IsCoinStake()) {
			if (IsFromMe(*ptx)) {
				...
				if (mapWallet.count(ptx->GetHash()))
				{
...
std::unique_ptr<WalletDatabase> MakeWalletDatabase(const std::string& name, const DatabaseOptions& options, DatabaseStatus& status, bilingual_str& error_string)
{
	...
	const fs::path& wallet_path = fs::absolute(fs::PathFromString(name), GetWalletDir());
...
bool CWallet::SelectCoinsForStaking(int64_t nTargetValue, unsigned int nSpendTime, std::set<std::pair<const CWalletTx*, unsigned int>> & setCoinsRet, int64_t & nValueRet) const
{
	...
	std::map<CTxDestination, CAmount> amounts;
	...
	for (auto & output : vCoins) {
		...
		auto it = amounts.find(addr);

		if (it == amounts.end()) {
			amounts[addr] = n;
		}
		else {
			amounts[addr] = amounts[addr] + n;
		}
	}
	...
	for (auto & output : vCoins) {
		...
		if (amounts.find(addr) != amounts.end()) {
			if (amounts[addr] < Params().GetConsensus().nStakeMinimumThreshold) {
				continue;
			}
		}
		else {
...
bool CWallet::CreateCoinStake(const FillableSigningProvider& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CMutableTransaction& txNew, CKey& key)
{
    // We need create new coin after current chain
	...
	if (txNew.vout.size() == 3) {
		txNew.vout[1].nValue = (blockValue / 2 / (COIN / 100)) * (COIN / 100);
	...
	for (auto & it : ratingRewards) {
		txNew.vout.push_back(it);
	}

	// Sign
	int nIn = 0;
	CMutableTransaction txNewConst(txNew);
	for (auto & coin : vwtxPrev) {
		bool signSuccess;
		const CScript& scriptPubKey = coin.first->tx->vout[coin.second].scriptPubKey;
		SignatureData sigdata;
		signSuccess = ProduceSignature(keystore, MutableTransactionSignatureCreator(&txNewConst, nIn, coin.first->tx->vout[coin.second].nValue, SIGHASH_ALL), scriptPubKey, sigdata);
	...
	if (nBytes >= (MAX_BLOCK_WEIGHT * 0.5) / 5) {
