...
#ifndef POCKETCOIN_CHAIN_H
#define POCKETCOIN_CHAIN_H
...
public:
    ...
    std::string ToString() const
    {
        return strprintf("CBlockIndex(pprev=%p, nHeight=%d, nMint=%s, nMoneySupply=%s, nFlags=(%s)(%d)(%s), nStakeModifier=%016x, hashProof=%s, prevoutStake=(%s), nStakeTime=%d merkle=%s, hashBlock=%s)",
            pprev, nHeight,
...
public:
	...
	void Crop(const CBlockIndex *pindex) {
		if ((*this).Contains(pindex)) {
			vChain.erase(vChain.begin() + pindex->nHeight, vChain.end());
		}
	}
};

#endif // POCKETCOIN_CHAIN_H
