...
const char * const POCKETCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //pocketcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", POCKETCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
    ...
    for (const auto& arg_map : m_available_args) {
        switch(arg_map.first) {
            ...
            case OptionsCategory::REGISTER_COMMANDS:
                ...
                break;
            case OptionsCategory::SQLITE:
                usage += HelpMessageGroup("SQLite parameters:");
                break;
            default:
                break;
        }
...
{
    return GetConfigFile(*this, GetPathArg("-conf", POCKETCOIN_CONF_FILENAME));
}
