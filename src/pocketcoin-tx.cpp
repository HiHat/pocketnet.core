...
static void SetupPocketcoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " pocketcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  pocketcoin-tx [options] <hex-tx> [commands]  Update hex-encoded pocketcoin transaction\n" +
            "or:     pocketcoin-tx [options] -create [commands]   Create hex-encoded pocketcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded pocketcoin transaction
