...
bool CNetAddr::SetI2P(const std::string& addr)
{
    ...
    bool invalid;
    const auto& address_bytes = DecodeBase32(b32_padded, &invalid);

    if (invalid) {
        return false;
    }

    if (address_bytes.size() != ADDR_I2P_SIZE) {
        return false;
    }
...
static std::string IPv6ToString_Old(Span<const uint8_t> a)
{
...
std::string CNetAddr::ToStringIP_Old() const
{
    switch (m_net) {
    case NET_IPV4:
    case NET_IPV6: {
        ...
        return IPv6ToString_Old(m_addr);
    }
    case NET_ONION:
        switch (m_addr.size()) {
