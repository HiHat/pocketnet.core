        ...
        ConsensusValidateResult Validate(const CTransactionRef& tx, const ScoreContentRef& ptx, const PocketBlockRef& block) override
        {
            ...
            auto[lastContentOk, lastContent] = PocketDb::ConsensusRepoInst.GetLastContent(
                *ptx->GetContentTxHash(),
                { CONTENT_POST, CONTENT_VIDEO, CONTENT_ARTICLE, CONTENT_STREAM, CONTENT_AUDIO, CONTENT_DELETE, BARTERON_OFFER, BARTERON_OFFER_PAID, APP }
            );
            ...
            if (!lastContentOk && block)
            {
                ...
                {
                    if (!TransactionHelper::IsIn(*blockTx->GetType(), {CONTENT_POST, CONTENT_VIDEO, CONTENT_ARTICLE, CONTENT_STREAM, CONTENT_AUDIO, CONTENT_DELETE, BARTERON_OFFER, BARTERON_OFFER_PAID, APP}))
                        continue;
    ...
    protected:
        ...
        virtual ConsensusValidateResult ValidateLimit(const ScoreContentRef& ptx, int count)
        {
            ...
            LogPrint(BCLog::CONSENSUS,
                "ScoreContentConsensus::ValidateLimit():  tx=%s type=%s adress=%s limit=%d/%d at height:%d\n",
