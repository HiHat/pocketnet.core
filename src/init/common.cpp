...
bool StartLogging(const ArgsManager& args)
{
    ...
    if (!LogInstance().StartLogging()) {
            return InitError(strprintf(Untranslated("Could not open debug log file %s"),
                fs::PathFromString(LogInstance().m_file_path)));
    }
