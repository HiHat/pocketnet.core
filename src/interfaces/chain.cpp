...
public:
    ...
    {
        return make_unique<NotificationsHandlerImpl>(std::move(notifications));
    }
    ...
    {
        return make_unique<RpcHandlerImpl>(command);
    }
...
std::unique_ptr<Chain> MakeChain(NodeContext& node) { return make_unique<ChainImpl>(node); }
