...
#include <qt/pocketcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Pocketcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new PocketcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new PocketcoinAddressCheckValidator(parent));
}

bool parsePocketcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no pocketcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("pocketcoin"))
        return false;
...
bool parsePocketcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parsePocketcoinURI(uriInstance, out);
}

QString formatPocketcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("pocketcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(PocketcoinUnits::format(PocketcoinUnits::PKOIN, info.amount, false, PocketcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openPocketcoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", POCKETCOIN_CONF_FILENAME));
    ...
    /* Open pocketcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Pocketcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Pocketcoin (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("pocketcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            ...
            optionFile << strprintf("Name=Pocketcoin (%s)\n", chain);
